...
// V9: INITIAL_ARCHIVE_DOCUMENTS now uses SEED_ARCHIVE_DOCUMENTS from staticData
const INITIAL_ARCHIVE_DOCUMENTS = SEED_ARCHIVE_DOCUMENTS;
...
Security Validation Access Passcode: pmo2026!`;
  ...
  const [tasks, setTasks] = useState<PMOTask[]>(() => {
    ...
    return saved ? JSON.parse(saved) : SEED_PMO_TASKS;
  });
  ...
  const [meetings, setMeetings] = useState<PMOMeeting[]>(() => {
    ...
    return saved ? JSON.parse(saved) : SEED_MEETINGS;
  });
  ...
  const resetToFactoryDefault = () => {
    if (confirm("Reset application to PMO factory baseline? This will clear custom tasks, SOPs, and chat logs.")) {
      ...
      setSelectedMeetingForPlayer(SEED_MEETINGS[0] || null);
